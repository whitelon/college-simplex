...
namespace Simplex
{
    ...
    {
        static readonly Regex _mainFunctionRegex = new Regex(@"(([+\-]?((?:\d*,)?\d+))[xX](\d+))+");
        static readonly Regex _restrictionRegex = new Regex(@"((([+\-]?((?:\d*,)?\d+))[xX](\d+))+)=((?:\d*,)?\d+)");
        static readonly Regex _memberRegex = new Regex(@"(([+\-]?((?:\d*,)?\d+))[xX](\d+))");

        
        public static double[] ParseMainFunction(string func)
        {
            func = func.Replace(" ", "");
            ...
            var MainCoefficient = new double[coeffs.Keys.Max()];
            foreach (var coeff in coeffs) MainCoefficient[coeff.Key - 1] = coeff.Value;
            return MainCoefficient;
        }
        public static Tuple<double[], double> ParseRestriction(string restr)
        {
            restr = restr.Replace(" ", "");
            ...
            var coeffs = ParseMainFunction(_restrictionRegex.Match(restr).Groups[1].Value);
